...
#[derive(Debug)]
pub struct JSNull<'ctx, 'rt> {
    ctx: &'ctx JSContext<'rt>,
    pub(crate) c_value: qjs_c::JSValue,
}


impl<'ctx, 'rt> Drop for JSNull<'ctx, 'rt> {
    fn drop(&mut self) {
...
impl<'ctx, 'rt> JSRawType for JSNull<'ctx, 'rt> {
    fn tag(&self) -> i32 {
